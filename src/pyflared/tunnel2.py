...
class TunnelManager:
    ...
    async def create_auto_tunnel(self, account_id: str) -> CreatedTunnel:
        return await create_tunnel(
            token=self.client.api_token,  # type: ignore
            account_id=account_id,
            tunnel_name=auto_tunnel_name(),
        )

    async def tunnel(self, mappings: Mappings) -> SecretStr:  # Tunnel Token
        if not mappings:
            raise Exception("No mappings provided")

        first_domain, _ = dict_first(mappings)

        zt = asyncio.create_task(self.mapped_zone(first_domain))
        ...
        tunnel = await self.create_auto_tunnel(first_zone.account.id)
...
class CreatedTunnel(BaseModel):
    ...
    def dns_record(self, domain: Domain) -> record_batch_params.CNAMERecordParam:
        return record_batch_params.CNAMERecordParam(
            name=domain,
            type="CNAME",
            content=f"{self.tunnel_id}{_cfargotunnel}",
            proxied=True,
            comment=_tag,
        )
...
async def create_tunnel(
    ...
    async with aiohttp.ClientSession() as session:
        async with session.post(url, headers=headers, json=payload) as response:
            ...
            return CreatedTunnel.from_cloudflare_response(x)
