...
@dataclass
class ProcessInstanceImpl(ProcessInstance):
...
class ProcessExecutor(AsyncContextManager[RunningProcess]):
    ...
    async def __aenter__(self) -> RunningProcess:
        ...
        pp = ProcesPlus(self.process, self.chunker, self.responders)
        active_iterator = await self.stack.enter_async_context(pp.de())
...
class ProcesPlus:
    ...
    def de(self):
        return self._build_pipeline().stream()
